...
\r
{"context":{"synthesis":{"audio":{"metadataoptions":{"sentenceBoundaryEnabled":"true","wordBoundaryEnabled":"false"},"outputFormat":"audio-24khz-48kbitrate-mono-mp3"}}}}\r
`
    );
    websocket.send(
      ssmlHeadersPlusData(
...
var SubMaker = class {
   ...
   */
  feed(msg) {
    if (msg.type !== "WordBoundary" || msg.type !== "SentenceBoundary" || msg.offset === void 0 || msg.duration === void 0 || msg.text === void 0) {
...
var EdgeTTS = class {
  ...
  async synthesize() {
    ...
    for await (const chunk of communicate.stream()) {
      ...
      } else if ((chunk.type === "WordBoundary"||chunk.type === "SentenceBoundary") && chunk.offset !== void 0 && chunk.duration !== void 0 && chunk.text !== void 0) {
        wordBoundaries.push({
          offset: chunk.offset,
...
var IsomorphicCommunicate = class {
  ...
  parseMetadata(data) {
    ...
    for (const metaObj of metadata["Metadata"]) {
      ...
      throw new UnknownResponse(`Unknown metadata type: ${metaType}, ${IsomorphicBuffer.toString(data)}`);
    }
    throw new UnexpectedResponse("No WordBoundary metadata found");
  }
...
var IsomorphicEdgeTTS = class {
  ...
  async synthesize() {
    ...
    for await (const chunk of communicate.stream()) {
      if (chunk.type === "audio" && chunk.data) {
        audioChunks.push(chunk.data);
      } else if ((chunk.type === "WordBoundary"||chunk.type === "WordBoundary") && chunk.offset !== void 0 && chunk.duration !== void 0 && chunk.text !== void 0) {
        wordBoundaries.push({
          offset: chunk.offset,
          duration: chunk.duration,
          text: chunk.text
            ...
            if (headers.Path === "audio.metadata") {
              try {
                ...
                if (metadata.Metadata && Array.isArray(metadata.Metadata)) {
                  const boundaries = metadata.Metadata.filter((item) => (item.Type === "WordBoundary"||item.Type === "SentenceBoundary") && item.Data).map((item) => ({
